      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
        <div>
          ...
          <p className="text-[var(--color-text-dim)] mt-2 text-lg font-light">
            Best moments and replays from your matches
          </p>
        </div>

        <button
          onClick={() => setIsAddModalOpen(true)}
          className="flex items-center gap-2 px-8 py-4 bg-[var(--color-primary)] text-black font-black uppercase tracking-wider text-sm rounded-xl hover:shadow-[0_0_20px_rgba(0,240,255,0.4)] transition-all transform hover:scale-105 active:scale-95 cursor-pointer"
        >
          ...
          {highlights.length === 0 ? (
            <div className="col-span-full py-24 text-center text-[var(--color-text-dim)] glass-panel rounded-3xl border border-[var(--color-border)] flex flex-col items-center justify-center">
              ...
              <p className="max-w-md mx-auto mb-8 font-light">
                Start building your library of epic moments. Add your first
                match replay or highlight clip now.
              </p>
              <button
                onClick={() => setIsAddModalOpen(true)}
                className="px-6 py-3 bg-white/5 hover:bg-white/10 text-[var(--color-primary)] font-bold rounded-xl transition-all border border-[var(--color-primary)]/20 hover:border-[var(--color-primary)]/50 cursor-pointer"
              >
                ...
                >
                  ...
                  <div className="relative aspect-video bg-black/50 group overflow-hidden">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center bg-black/20 group-hover:bg-black/40 transition-colors backdrop-blur-[2px] group-hover:backdrop-blur-none">
                      <button
                        ...
                        className="w-14 h-14 rounded-full bg-[var(--color-primary)] flex items-center justify-center text-black shadow-[0_0_20px_rgba(0,240,255,0.4)] transform scale-90 group-hover:scale-110 transition-transform cursor-pointer"
                      >
                  ...
                  <div className="p-5 flex-1 flex flex-col">
                    <div className="flex justify-between items-start gap-4 mb-3">
                      <h3 className="font-bold text-lg leading-snug line-clamp-2 text-[var(--color-text-main)] group-hover:text-[var(--color-primary)] transition-colors">
                        {item.title}
                      </h3>
                      <button
                        onClick={() => handleDelete(item.id)}
                        className="text-[var(--color-text-dim)] hover:text-red-500 transition-colors p-1 cursor-pointer"
                      >
                        <Trash2 size={16} />
                      </button>
                    </div>
            ...
            >
              ...
              <div className="p-6 border-b border-white/5 flex justify-between items-center bg-white/5">
                ...
                <button
                  ...
                  className="text-[var(--color-text-dim)] hover:text-white transition-colors bg-white/5 hover:bg-white/10 p-2 rounded-full cursor-pointer"
                >
                  <X size={20} />
                </button>
              </div>
      ...
      <AnimatePresence>
        {selectedVideo && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-50 flex items-center justify-center p-4 md:p-10 bg-black/95 backdrop-blur-xl cursor-pointer"
            onClick={(e) => {
          ...
          >
            <div className="w-full max-w-5xl aspect-video bg-black rounded-3xl overflow-hidden shadow-2xl relative border border-white/10 ring-1 ring-white/5">
              <button
                ...
                className="absolute top-4 right-4 z-10 text-white/70 hover:text-white bg-black/50 hover:bg-red-500/80 rounded-full p-2 transition-all backdrop-blur-md cursor-pointer"
              >
                <X size={24} />
              </button>
              ...
              getYoutubeId(selectedVideo.url) ? (
                ...
                <div className="flex flex-col items-center justify-center h-full text-center p-8">
                  <p className="mb-4 text-xl">
                    We cannot direct embed this video type.
                  </p>
                  <a
